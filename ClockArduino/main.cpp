...
//  ClockArduino
//
//  Created by Patryk Piwowarczyk on 10/01/2022.
//
...
SevSeg Display;
const double period = 1; //Change to make it count fasterone second
const double ledFlash = 500; //LED blinking time

const int hrButton = A0; //connect buttons with according pins
const int minButton = A1;
const int ledPin = A2;

double startCounter;
double led_startCounter;
double currentTime;
double led_currentTime;

int Hrs = 0; // set starting time
int Min = 0;
int Sec = 0;
int Time;
int ledState = LOW;
...
void setup(){
  ...
  bool leadingZeros = true; // if the hour is 04 for ex.
  bool disableDecPoint = true; //we dont use dot to separate anything
  bool resistorsOnSegments = false; // we use our own resistors
  bool updateWithDelays = false; // no delays for us.
  
 
  byte numDigits = 4; // storing 4 digits
  byte digitPins[] = {10, 11, 12, 13}; //connecting with pins
  byte segmentPins[] = {2, 3, 4, 5, 6, 7, 8, 9}; //pins
...
void loop(){
  ...
  currentTime = millis();
  if (currentTime - startCounter >= period){
  ...
  if (led_currentTime - led_startCounter >= ledFlash){
    led_startCounter = led_currentTime;
    if (ledState == LOW){
      ledState = HIGH;
