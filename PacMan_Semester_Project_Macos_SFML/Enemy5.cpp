...
double deltaTime5 = 0;
int iRand5;
Enemy5::Enemy5(float X,float Y,float W,float H, sf::Texture& t){
    ...
    setTexture(t);
    setPosition(2432, 896);

}
void Enemy5::update(bool &enemyRight, bool &enemyLeft, bool &enemyUp, bool &enemyDown,  std::vector<Platform>& level)
{
    

    float secondsToDelay = 1;

        deltaTime5 = (clock() - TimeZero5) / (CLOCKS_PER_SEC/12);

    if(deltaTime5 > secondsToDelay)
    {
    iRand5 = (rand() % 4)+1;
...
void Enemy5::collide(float xvel,float yvel, std::vector<Platform>& level){
