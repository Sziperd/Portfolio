...
double deltaTime2 = 0;
int iRand2;
Enemy2::Enemy2(float X,float Y,float W,float H, sf::Texture& t){
    ...
    setTexture(t);
    setPosition(1536, 1536);

}
void Enemy2::update(bool &enemyRight, bool &enemyLeft, bool &enemyUp, bool &enemyDown,  std::vector<Platform>& level)
{
    

    float secondsToDelay = 1;

        deltaTime2 = (clock() - TimeZero2) / (CLOCKS_PER_SEC/10);

    if(deltaTime2 > secondsToDelay)
    {
    iRand2 = (rand() % 4)+1;
...
void Enemy2::collide(float xvel,float yvel, std::vector<Platform>& level){
