...
double deltaTime4 = 0;
int iRand4;
Enemy4::Enemy4(float X,float Y,float W,float H, sf::Texture& t){
...
void Enemy4::update(bool &enemyRight, bool &enemyLeft, bool &enemyUp, bool &enemyDown,  std::vector<Platform>& level)
{
    

    float secondsToDelay = 1;

        deltaTime4 = (clock() - TimeZero4) / (CLOCKS_PER_SEC/8);

    if(deltaTime4 > secondsToDelay)
    {
    iRand4 = (rand() % 4)+1;
...
void Enemy4::collide(float xvel,float yvel, std::vector<Platform>& level){
