...
int main(int argc, const char * argv[]) {
    ...
    int matrix1[3][3]= {{-5,-3,-1},{4,5,8},{4,2,-2}};  // input game values
    int matrix2[3][3]= {{-1,4,4},{2,2,-3},{0,-6,7}};  // same but for the second player...
    
    
    // setting corresponding variables to each table value
    //basically replicating the given tables in code..
    ...
    int maxc1;  //integers for saving found max value of each column
    int maxc2;
    int maxc3;
    ...
    int memory5;
    int memory6;
    string matrixx1;  //just for a cout
    string matrixx2;
    string matrixx3;
    
    
    
    //code for checking each row's max value... basically comparing each value in a column and finding the maximal one.
    ...
    //integers for saving row-maximum  number
    ...
    // same as above, code to determine the maximum number in each row by comparing every row's number..
    ...
    //check whitch [x][y] double..
    //Basically if the value is the same it means that there is an equilibrum point
    // in short, same max row and column for each player = equilibrum point, the payoff is the highest for //both of them in the same place
    ...
    //second part of the program for maxmin strategy:
    ...
    int num1 = 0; // initializing zero value for the beggining
    
    string matrixxx1; // just for the text outputs..
    string matrixxx2;
    string matrixxx3;
    
    // checking the smallest number in each row.
    ...
    int maxminp1; // variable that the maximum of the minimum will be set as
    
    // finally finding the maximum of the minimums by comparing them
...
//just for output
    ...
    //basically exactly the same code but this time for the second player so we are working with columns instead of rows and vice versa.
